...
const coreIds=new Set(['drive','youtube','text','database']);const coreReady=checks.filter((item)=>coreIds.has(item.id)).every((item)=>item.state==='READY');const result={coreReady,textProvider,searchProvider,voiceProvider,imageProvider,videoProvider,checks};
if(jsonMode)console.log(JSON.stringify(result,null,2));else{console.log(`AUTO-YTB connections: ${coreReady?'CORE READY':'CORE BLOCKED'}\n`);for(const item of checks){const suffix=item.missing.length?` — missing: ${item.missing.join(', ')}`:'';console.log(`${item.state.padEnd(12)} ${item.label}${suffix}`);}console.log('\nGemini can provide search grounding, TTS, images and Veo video using the same GEMINI_API_KEY. Social platforms remain optional per Series Automation Profile.');}
