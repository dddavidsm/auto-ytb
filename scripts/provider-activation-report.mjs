...
const migrationNames = ['024_durable_production_runs.sql', '025_real_production_worker.sql'];
const migration = { status: 'MIGRATION_REQUIRED', migrations: migrationNames, connection: process.env.DATABASE_URL ? 'UNVERIFIED' : 'MISSING', missing: migrationNames };
if (process.env.DATABASE_URL) {
  const db = new NodePostgresSqlClient(process.env.DATABASE_URL, { ssl: process.env.DATABASE_SSL === 'true' ? { rejectUnauthorized: false } : undefined });
  try {
    ...
    migration.missing = migrationNames.filter((filename) => !applied.has(filename));
    migration.status = migration.missing.length ? 'MIGRATION_REQUIRED' : 'APPLIED';
    migration.connection = 'AVAILABLE';
  } catch (error) {
    migration.connection = 'UNAVAILABLE';
    migration.reason = `Database probe failed${error?.code ? ` (${error.code})` : ''}.`;
  } finally {
    await db.close();
  }
}
const capabilities = buildCapabilityActivationReport(registry, process.env);
const report = { generatedAt: new Date().toISOString(), migration, requirements: buildProviderCredentialRequirements(registry, process.env), capabilities, probes, minimumStack: { research: 'TAVILY_API_KEY or approved local research input', text: 'OPENAI_API_KEY or local-template', vision: 'OPENAI_API_KEY/GEMINI_API_KEY; otherwise NOT_EVALUATED', image: 'GEMINI_API_KEY/RUNWAY_API_KEY; otherwise local programmatic fallback', video: 'Optional: GEMINI_API_KEY/RUNWAY_API_KEY; otherwise image-motion fallback', tts: 'VOICE_API_KEY (or legacy ELEVENLABS_API_KEY)/GEMINI_API_KEY; otherwise windows-sapi-local' } };
