...
try {
  const publications = (await db.query(`select p.id,p.youtube_video_id,p.channel_id,coalesce(pr.total_cost_usd,0)::float as production_cost_usd from publications p left join production_runs pr on pr.id=p.production_run_id where p.youtube_video_id is not null and p.state in ('private','reviewed','scheduled','public') order by p.updated_at desc`)).rows;
  for (const pub of publications) {
    ...
    console.log(`${pub.youtube_video_id}: ${perf.views} views, ${perf.averageViewPercentage.toFixed(1)}% avg viewed, $${perf.revenueUsd.toFixed(2)} revenue`);
  }
