...
try{
  const row=(await db.query(`
    select p.id as publication_id,p.youtube_video_id,p.state,
      q.score::float as qa_score,q.blockers,q.report,
      r.research_confidence::float as research_confidence
    from production_runs pr
    left join publications p on p.production_run_id=pr.id
    left join lateral (select score,blockers,report from qa_reports where production_run_id=pr.id order by created_at desc limit 1) q on true
    left join content_ideas ci on ci.id=pr.content_idea_id
    left join lateral (select research_confidence from research_dossiers rd where rd.opportunity_id=ci.opportunity_id order by rd.created_at desc limit 1) r on true
  ...
  const decision=decideAutonomousPublication({
    ...
    qaScore:Number(row.qa_score??0),researchConfidence:Number(row.research_confidence??0),qaBlockers:row.blockers??[],unresolvedRights:rights.rows.length,policyWarnings,youtubeVideoId:row.youtube_video_id,
  });
  if(decision.action==='SCHEDULE'&&row.publication_id&&decision.publishAt){
    ...
    const inserted=await db.query(`insert into jobs (job_key,kind,state,priority,max_attempts,payload) values ($1,'schedule_publication','queued',98,4,$2::jsonb) on conflict (job_key) do nothing returning id`,[jobKey,JSON.stringify({publicationId:row.publication_id,publishAt:decision.publishAt,source:'full-autonomous'})]);
    if(inserted.rows[0])await db.query(`insert into job_events (job_id,event_type,detail) values ($1,'autonomous_publish_scheduled',$2::jsonb)`,[inserted.rows[0].id,JSON.stringify({productionRunId,publicationId:row.publication_id,publishAt:decision.publishAt,reasons:decision.reasons})]);
    console.log(JSON.stringify({...decision,jobId:inserted.rows[0]?.id??null,productionRunId,publicationId:row.publication_id},null,2));
  } else {
    await db.query(`update production_runs set metadata=metadata||$2::jsonb,updated_at=now() where id=$1`,[productionRunId,JSON.stringify({autonomousPublication:{...decision,evaluatedAt:new Date().toISOString()}})]);
    console.log(JSON.stringify({...decision,productionRunId,publicationId:row.publication_id??null},null,2));
  }
