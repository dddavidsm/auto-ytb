...
async function execute(job){
  ...
  if(job.kind==='produce_opportunity'){
    ...
    await runNode('scripts/live-pipeline.mjs',[`--topic=${topic}`,`--opportunity-id=${job.opportunity_id}`,`--format=${contentFormat}`]);
    ...
    return {actualCostUsd:Number(costResult.rows[0]?.cost ?? 0),contentFormat};
  }
