...
function productionProfileFromMetrics(metrics, base){
  ...
  let costFactor=1;
  const guidance=[];
  if(hook!=null && hook<0.55){durationFactor-=0.05;targetSceneDurationSec=8;guidance.push('Front-load the viewer promise and stakes. The hook should establish the core tension quickly and avoid contextual throat-clearing.');}
  else if(hook!=null && hook>=0.75) guidance.push('Early retention is strong. Keep the opening concise, but preserve room for a controlled curiosity gap after the promise is clear.');
  if(avp!=null && avp<45){durationFactor-=0.10;targetSceneDurationSec=Math.min(targetSceneDurationSec,8);guidance.push('Recent average percentage viewed is weak. Compress exposition, shorten transitions, and increase visual/narrative progression density.');}
  else if(avp!=null && avp>=60){durationFactor+=0.08;targetSceneDurationSec=11;guidance.push('Recent average percentage viewed is strong. Allow modestly deeper development where it adds payoff, without adding filler.');}
  if(roi!=null && roi>1){costFactor=1.15;guidance.push('Recent production economics are positive. Spend selectively on high-impact explanatory or reveal moments, not uniformly across the video.');}
  else if(roi!=null && roi<0){costFactor=0.82;targetSceneDurationSec=Math.max(targetSceneDurationSec,10);guidance.push('Recent production economics are negative. Prefer lower-cost visual solutions unless a premium generated shot clearly improves the hook, reveal, or payoff.');}
...
try {
  ...
  const learningResult=await db.query(`select count(distinct publication_id)::int as sample_size, avg(case when feature_key='strongHook' then case when feature_value #>> '{}'='true' then 1.0 else 0.0 end end)::float as strong_hook_rate, avg(case when feature_key='averageViewPercentage' then nullif(feature_value #>> '{}','')::numeric end)::float as average_view_percentage, avg(case when feature_key='shareRate' then nullif(feature_value #>> '{}','')::numeric end)::float as share_rate, avg(case when feature_key='economics' then nullif(feature_value->>'roi','')::numeric end)::float as average_roi from learning_signals where channel_id=$1 and observed_at >= now()-interval '120 days'`,[channelId]);
  const attributeRows=(await db.query(`select publication_id,feature_key,feature_value,strength from learning_signals where channel_id=$1 and signal_type='packaging_attribute_performance' and observed_at>=now()-interval '180 days'`,[channelId])).rows;
  ...
  const learnedProfile=productionProfileFromMetrics(learningMetrics,{targetDurationSec:baseTargetDurationSec,maxCostUsd:baseMaxCostUsd});
  const opportunityId=arg('opportunity-id',null);
  const ideaRow=await db.query(`insert into content_ideas (opportunity_id,format,working_title,premise,target_viewer,hook_hypothesis,status) values ($1,'long',$2,$3,$4,$5,'production') returning id`,[opportunityId,topic,topic,channel.targetViewer,learnedProfile.scriptGuidance ?? packagingGuidance ?? 'Generated after research']);
  const contentIdeaId=ideaRow.rows[0].id;
  const structuralExperiment=selectStructuralExperiment({sampleSize:Number(learningMetrics?.sample_size ?? 0),experimentSeed:`${channelId}:${contentIdeaId}:${topic}`,allowCostExperiment:process.env.ALLOW_COST_EXPERIMENTS!=='false'});
  const arm=structuralExperiment.selected;
  const productionProfile={
    ...learnedProfile,
    targetDurationSec:Math.round(Math.max(480,Math.min(900,learnedProfile.targetDurationSec*arm.targetDurationFactor))),
    targetSceneDurationSec:Math.round(Math.max(5,Math.min(16,learnedProfile.targetSceneDurationSec*arm.targetSceneDurationFactor))*10)/10,
    maxCostUsd:Math.round(Math.max(8,Math.min(baseMaxCostUsd*1.25,learnedProfile.maxCostUsd*arm.maxCostFactor))*100)/100,
  ...
  if(result.manifest?.script){
    ...
    await db.query(`insert into model_experiments (channel_id,experiment_type,hypothesis,variant_a,variant_b,variant_c,winner,outcome) values ($1,'packaging_bandit',$2,$3::jsonb,$4::jsonb,$5::jsonb,$6,$7::jsonb)`,[channelId,'Bounded exploration/exploitation improves packaging outcome while preserving periodic exploration.',JSON.stringify(variants[0] ?? {}),JSON.stringify(variants[1] ?? {}),JSON.stringify(variants[2] ?? {}),result.manifest.selectedPackagingId,JSON.stringify({productionRunId,selection:result.manifest.packagingSelection ?? null,status:'assigned'})]);
    await db.query(`insert into model_experiments (channel_id,experiment_type,hypothesis,variant_a,winner,outcome) values ($1,'structural_bandit',$2,$3::jsonb,$4,$5::jsonb)`,[channelId,'One-axis bounded structural exploration can improve retention and unit economics without confounding multiple strong changes.',JSON.stringify(structuralExperiment),arm.arm,JSON.stringify({productionRunId,axis:arm.axis,arm:arm.arm,status:'assigned'})]);
  ...
  console.log(JSON.stringify({productionRunId,state:result.state,qa:result.qa?.score,costUsd:result.manifest?.actualCostUsd,thumbnails:result.manifest?.thumbnails.length ?? 0,renderUri:result.renderUri,youtubeVideoId:result.externalId ?? null,learnedPackaging:Boolean(packagingGuidance),productionProfile,packagingSelection:result.manifest?.packagingSelection ?? null,structuralExperiment},null,2));
} catch(error) {
