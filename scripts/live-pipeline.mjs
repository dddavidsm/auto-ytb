...
function productionProfileFromMetrics(metrics,base,format){
  const sample=Number(metrics?.sample_size??0),short=format==='SHORT_VERTICAL',durationBounds=short?{min:20,max:180}:{min:480,max:900},sceneBounds=short?{min:2.5,max:8,base:4.5}:{min:5,max:16,base:10},costFloor=short?2:8;
...
try{
  ...
  const packagingLearning=packagingLearningFromRows(attributeRows),structuralLearning=structuralLearningFromRows(structuralRows),creativeLearning=buildCreativeLearningGuidance(creativeRows),learningMetrics=learningResult.rows[0];
  const packagingGuidance=packagingGuidanceFromMetrics(learningMetrics);
  const baseTargetDurationSec=isShort?Number(channel.shortTargetDurationSec||45):Number(channel.targetDurationSec||660),configuredMax=Number(process.env.MAX_PRODUCTION_COST_USD||channel.maxProductionCostUsd||18),baseMaxCostUsd=isShort?Math.min(configuredMax,Number(channel.shortMaxProductionCostUsd||Math.max(4,configuredMax*0.35))):configuredMax;
  ...
  if(result.qa)await productionRepo.addQaReport({productionRunId,passed:result.qa.passed,score:result.qa.score,containsSyntheticMedia:result.qa.containsSyntheticMedia,blockers:result.qa.blockers,report:{...result.qa,attention:result.attention??null,finalInspection:result.finalInspection??null}});
  ...
  console.log(JSON.stringify({productionRunId,contentFormat,state:result.state,qa:result.qa?.score,attention:result.attention?.score,renderQa:result.finalInspection?.score,costUsd:result.manifest?.actualCostUsd,thumbnails:result.manifest?.thumbnails.length??0,renderUri:result.renderUri,youtubeVideoId:result.externalId??null,learnedPackaging:Boolean(packagingGuidance),creativeGuidanceWinners:creativeLearning.winners,productionProfile,packagingSelection:result.manifest?.packagingSelection??null,structuralLearning,structuralExperiment},null,2));
}catch(error){if(productionRunId)await db.query(`update production_runs set state='BLOCKED',metadata=metadata||$2::jsonb,updated_at=now() where id=$1`,[productionRunId,JSON.stringify({error:error instanceof Error?error.message:String(error),contentFormat})]);throw error;}finally{await db.close();}
