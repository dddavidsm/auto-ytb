...
try{
  ...
  console.log(JSON.stringify({productionRunId,contentFormat,contentArchetype:result.manifest?.contentArchetype?.id??runtime.archetypeDecision?.archetype??null,executionPlan:result.manifest?.executionPlan??null,state:result.state,qa:result.qa?.score,attention:result.attention?.score,renderQa:result.finalInspection?.score,costUsd:result.manifest?.actualCostUsd,thumbnails:result.manifest?.thumbnails.length??0,renderUri:result.renderUri,youtubeVideoId:result.externalId??null,learnedPackaging:Boolean(packagingGuidance),creativeLearningScope:creativeLearning.scope,creativeGuidanceWinners:creativeLearning.winners,productionProfile,packagingSelection:result.manifest?.packagingSelection??null,structuralLearning,structuralExperiment},null,2));
}catch(error){if(productionRunId)await db.query(`update production_runs set state='BLOCKED',metadata=metadata||$2::jsonb,updated_at=now() where id=$1`,[productionRunId,JSON.stringify({error:error instanceof Error?error.message:String(error),contentFormat,contentArchetype:runtime.archetypeDecision??null})]);throw error;}finally{await db.close();}
