...
try{
  ...
  const targetSceneDurationSec=Number(row.metadata?.productionProfile?.targetSceneDurationSec||10);
  const planned=planScenes(row.script,{targetSceneDurationSec});
  ...
  const fingerprint=extractCreativeFingerprint({contentFormat,script:row.script,scenes,packaging,selectedPackagingId,attentionScore});
  ...
  console.log(JSON.stringify({productionRunId,publicationId:row.publication_id??null,contentFormat,attentionScore:attentionScore??null,hook:fingerprint.hookRetentionDevice,narrativeArchetype:fingerprint.narrativeArchetype,scenes:fingerprint.sceneCount,visualMix:fingerprint.visualMix},null,2));
