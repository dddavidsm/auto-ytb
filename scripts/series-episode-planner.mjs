...
try{
  ...
  for(const row of rows){
    ...
    const decision=String(row.decision??'LEARN'),confidence=Number(row.confidence??0);
    ...
    for(const seed of seeds){
      ...
      created.push({seriesId:row.id,seriesKey:row.series_key,opportunityId:opportunity.id,title,decision,score:Math.min(96,baseScore)});
      break;
    }
  }
  console.log(JSON.stringify({created:created.length,episodes:created},null,2));
