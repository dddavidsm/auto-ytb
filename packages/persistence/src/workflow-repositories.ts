...
export class ProductionRepository {
  ...
  async updateRun(id:string,input:{state:string;totalCostUsd?:number;metadata?:unknown}):Promise<void>{await this.db.query(`update production_runs set state=$2,total_cost_usd=coalesce($3,total_cost_usd),metadata=coalesce($4::jsonb,metadata),updated_at=now() where id=$1`,[id,input.state,input.totalCostUsd??null,input.metadata===undefined?null:JSON.stringify(input.metadata)]);}
...
export class PublicationRepository {
  constructor(private readonly db:SqlClient){}
  async create(input:{productionRunId?:string|null;channelId:string;youtubeVideoId?:string|null;state:'rendered'|'private'|'reviewed'|'scheduled'|'public'|'failed';publishAt?:Date|null;containsSyntheticMedia:boolean;metadata?:unknown}):Promise<string>{const r=await this.db.query<{id:string}>(`insert into publications (production_run_id,channel_id,youtube_video_id,state,publish_at,contains_synthetic_media,metadata) values ($1,$2,$3,$4,$5,$6,$7::jsonb) returning id`,[input.productionRunId??null,input.channelId,input.youtubeVideoId??null,input.state,input.publishAt??null,input.containsSyntheticMedia,JSON.stringify(input.metadata??{})]);if(!r.rows[0])throw new Error('Publication insert returned no row');return r.rows[0].id;}
