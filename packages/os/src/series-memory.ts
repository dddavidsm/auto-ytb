...
export type SeriesPerformanceEvidence={
  sampleSize:number;
  views:number;
  averageViewPercentage?:number|null;
  averageViewDurationSeconds?:number|null;
  shareRate?:number|null;
  subscribersPerThousand?:number|null;
  revenueUsd?:number|null;
  costUsd?:number|null;
  profitUsd?:number|null;
  roi?:number|null;
  watchMinutesPerDollar?:number|null;
  qualityReportCoverage?:number|null;
  averageVisualContinuityScore?:number|null;
  averageKidsQualityScore?:number|null;
  qualityWarningRate?:number|null;
...
export function decideSeriesContinuation(evidence:SeriesPerformanceEvidence):SeriesContinuationDecision{
  ...
  const qualityCoverage=evidence.qualityReportCoverage==null?null:Math.max(0,Math.min(1,Number(evidence.qualityReportCoverage)));
  const visual=evidence.averageVisualContinuityScore==null?null:clamp(Number(evidence.averageVisualContinuityScore));
  const kids=evidence.averageKidsQualityScore==null?null:clamp(Number(evidence.averageKidsQualityScore));
  const warningRate=evidence.qualityWarningRate==null?null:Math.max(0,Math.min(1,Number(evidence.qualityWarningRate)));
  const blocked=Math.max(0,Math.floor(Number(evidence.qualityBlockedCount??0)));
  const qualityPenalty=(visual==null?0:Math.max(0,82-visual)*0.18)+(kids==null?0:Math.max(0,82-kids)*0.16)+(warningRate==null?0:warningRate*5)+(blocked?Math.min(18,blocked*8):0);
  const score=Math.round(clamp(50+(avp-50)*0.55+share*1.3+subs*0.15+roi*4+Math.min(8,Math.log10(watch+1)*2.2)-qualityPenalty)*10)/10;
  const rationale=[`AVP ${avp.toFixed(1)}%`,`ROI ${roi.toFixed(2)}x`,`share rate ${share.toFixed(2)}%`,`n=${n}, views=${Math.round(views)}`];
  if(qualityCoverage!=null)rationale.push(`quality coverage ${(qualityCoverage*100).toFixed(0)}%`);
  if(visual!=null)rationale.push(`visual continuity ${visual.toFixed(1)}/100`);
  if(kids!=null)rationale.push(`kids quality ${kids.toFixed(1)}/100`);
  if(warningRate!=null)rationale.push(`quality warning rate ${(warningRate*100).toFixed(0)}%`);
  ...
  const severeQualityRisk=blocked>0||(visual!=null&&visual<70)||(kids!=null&&kids<72);
  if(severeQualityRisk)return{decision:'REVIEW',score,confidence,rationale:[...rationale,'Quality evidence is too weak for autonomous expansion even if audience metrics are attractive.']};
  const scaleQualityReady=(qualityCoverage==null||qualityCoverage>=0.8)&&(visual==null||visual>=82)&&(kids==null||kids>=82)&&(warningRate==null||warningRate<=0.35);
  if(n>=6&&views>=1500&&score>=68&&roi>=0.4&&scaleQualityReady)return{decision:'SCALE',score,confidence,rationale:[...rationale,'Series has repeatable audience, economic and quality evidence strong enough to increase cadence/budget cautiously.']};
  if(n>=6&&views>=1500&&score>=68&&roi>=0.4&&!scaleQualityReady)return{decision:'CONTINUE',score,confidence,rationale:[...rationale,'Audience/economics support the series, but quality evidence is not yet strong enough to scale cadence or budget.']};
