import type { PackagingVariant, ProductionContentFormat, Scene, VideoScript } from '@auto-ytb/production';
...
const CURIOSITY = /\b(but|except|until|hidden|really|actually|why|what if|nobody|secret|problem|changed|turns out|the catch|instead|unexpected|race|risk|mystery|could|might)\b/i;
...
export function reviewAttentionBlueprint(input:{
  ...
  const opening=input.script.beats.slice(0,isShort?1:2).map((beat)=>beat.narration).join(' ');
  ...
  if(CURIOSITY.test(first?.narration??''))hookScore+=10;
  if(!HOUSEKEEPING.test(first?.narration??''))hookScore+=10; else hookScore-=25;
  ...
  const hasEvidence=purposeOrder.includes('evidence')||purposeOrder.includes('setup');
  ...
  const storyArcScore=clamp((hasHook?24:0)+(hasEvidence?18:0)+(hasEscalation?18:0)+(hasReveal?20:0)+(hasPayoff?20:0));
  ...
  if(selected && lexicalCoverage(selected.promise??selected.title,input.script.beats[payoffIndex]?.narration??'')>0.18)payoffScore+=5;
  payoffScore=clamp(payoffScore);

  const sentence=sentenceStats(narration);
  ...
  if(HOUSEKEEPING.test(narration))clarityScore-=10;
  ...
  const dimensions:AttentionDimension[]=[
    ...
    {id:'hook-strength',score:round(hookScore),weight:18,message:`Hook ${firstHookSeconds===999?'missing':`${firstHookSeconds.toFixed(1)}s`} · ${first?.retentionDevice??'no retention device'}`},
    ...
    {id:'story-arc',score:round(storyArcScore),weight:12,message:`Hook ${hasHook?'yes':'no'} · escalation ${hasEscalation?'yes':'no'} · reveal/payoff ${hasPayoff?'yes':'no'}`},
    ...
    {id:'clarity',score:round(clarityScore),weight:6,message:`${averageWordsPerSentence.toFixed(1)} words/sentence average`},
    {id:'format-fit',score:round(formatFitScore),weight:5,message:`${input.contentFormat} · ${scriptDuration}s`},
  ];
  ...
  if(hookScore<75)add('critical','weak-hook','Opening is not strong enough for the format.',isShort?'Make the first spoken/visual beat deliver tension or an irresistible question in the first 1-2 seconds; remove all setup before the hook.':'Open directly on the highest-stakes contradiction, surprising consequence or unresolved question; delay background/context until the promise is clear.');
  if(promiseScore<70)add('critical','promise-mismatch','The title/thumbnail promise is not paid into quickly enough.','Rewrite the opening so the viewer immediately understands why the clicked promise matters, without restating the title verbatim.');
  if(storyArcScore<75)add('major','flat-arc','The narrative lacks a complete escalation/reveal/payoff progression.','Reorder beats into hook → essential context/evidence → escalation/complication → reveal → payoff.');
  if(momentumScore<72)add('major','low-momentum','Too much of the script lacks curiosity, contrast or progressive revelation.','Give each beat a reason to continue: a concrete question, contrast, consequence, reveal or unresolved information gap.');
  if(visualCommunicationScore<72)add('critical','weak-visual-storytelling','Visual plan is too static, repetitive or underspecified.','Make every visual earn its screen time by explaining, proving, escalating or refreshing attention; replace decorative repeats with evidence, diagrams, motion or meaningful scene changes.');
  ...
  if(payoffScore<72)add('critical','weak-payoff','The video does not clearly resolve the curiosity it creates.','Strengthen the final reveal/payoff so the viewer receives a concrete answer, consequence or emotional resolution before any CTA.');
  if(clarityScore<70)add('major','dense-language','Narration is harder to process than necessary.','Shorten sentences, remove stacked clauses and make each sentence advance one clear idea.');
