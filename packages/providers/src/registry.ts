...
const hasAll = (env: Record<string, string | undefined>, ...keys: string[]) => keys.every((key) => Boolean(env[key]?.trim()));
...
export function createDefaultProviderRegistry(env: Record<string, string | undefined> = process.env): ProviderRegistry {
  const higgsfieldModel = env.HIGGSFIELD_VIDEO_MODEL ?? env.HF_VIDEO_MODEL ?? 'wan/v2.7/text-to-video';
  const higgsfieldSeedance25 = higgsfieldModel.startsWith('bytedance/seedance-2.5/');
  const higgsfieldSeedance20 = higgsfieldModel.startsWith('bytedance/seedance-2.0/');
  const higgsfieldReferenceCapable = higgsfieldSeedance25 || higgsfieldSeedance20 || /\/(image|reference)-to-video$|\/video-edit$/.test(higgsfieldModel);
  const higgsfieldEstimatedRate = Number(env.HIGGSFIELD_USD_PER_SECOND ?? (higgsfieldSeedance25 ? 0.144 : higgsfieldSeedance20 ? 0.0985 : 0.1));
  return new ProviderRegistry([
    ...
    descriptor({ provider: 'higgsfield', model: higgsfieldModel, capabilities: ['VIDEO'], qualityScore: 90, estimatedUnitCostUsd: Number.isFinite(higgsfieldEstimatedRate) && higgsfieldEstimatedRate > 0 ? higgsfieldEstimatedRate : 0.15, latencyClass: 'STANDARD', reliability: 88, maxDurationSeconds: higgsfieldSeedance25 ? 30 : 15, resolutionSupport: higgsfieldSeedance25 ? ['480p', '720p'] : higgsfieldSeedance20 ? ['480p', '720p', '1080p', '4k'] : ['720p', '1080p'], aspectRatios: higgsfieldSeedance25 || higgsfieldSeedance20 ? ['16:9', '9:16', '1:1', '4:3', '3:4', '21:9'] : ['16:9', '9:16', '1:1', '4:3', '3:4'], referenceImageSupport: higgsfieldReferenceCapable, characterConsistencySupport: higgsfieldReferenceCapable, seedSupport: !higgsfieldSeedance25 && !higgsfieldSeedance20, commercialUsageNotes: 'Official Higgsfield API adapter. Pricing is an estimate unless HIGGSFIELD_USD_PER_SECOND is configured; retain model, cost and provenance metadata.', credentialStatus: has(env, 'HF_CREDENTIALS') || hasAll(env, 'HF_API_KEY_ID', 'HF_API_KEY_SECRET') ? 'LIVE' : 'NO_CREDENTIALS', enabled: true }),
  ]);
}
