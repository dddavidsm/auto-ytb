export type ProviderCapabilityName = 'TEXT' | 'VISION' | 'IMAGE' | 'VIDEO' | 'TTS' | 'MUSIC' | 'SFX' | 'CAPTIONS' | 'RENDER';
export type CredentialStatus = 'LIVE' | 'LOCAL' | 'FIXTURE' | 'NO_CREDENTIALS' | 'PROVIDER_UNAVAILABLE';
export type ProviderDescriptor = { provider: string; model?: string; capabilities: ProviderCapabilityName[]; qualityScore: number; estimatedUnitCostUsd: number; latencyClass: 'FAST' | 'STANDARD' | 'SLOW'; reliability: number; maxDurationSeconds?: number; resolutionSupport: string[]; aspectRatios: string[]; referenceImageSupport: boolean; characterConsistencySupport: boolean; commercialUsageNotes: string; credentialStatus: CredentialStatus; enabled: boolean };
export type ProviderRouteRequest = { capability: ProviderCapabilityName; qualityTarget?: number; budgetRemainingUsd?: number; preferredProvider?: string; format?: string; aspectRatio?: string; durationSeconds?: number; requireCharacterConsistency?: boolean };
...
export class ProviderRouter {
  ...
  route(input: ProviderRouteRequest): ProviderRoute | null {
    ...
    const ranked = candidates.map((provider) => {
      ...
      const preference = input.preferredProvider && input.preferredProvider === provider.provider ? 18 : 0;
      ...
      return { ...provider, routeScore, reason: preference ? 'channel preference plus capability, quality, cost and credential match' : 'best available capability, quality, cost and credential match' };
...
export function createDefaultProviderRegistry(env: Record<string, string | undefined> = process.env): ProviderRegistry {
  return new ProviderRegistry([
    ...
    descriptor({ provider: 'tavily', model: 'search', capabilities: ['TEXT', 'VISION'], qualityScore: 72, estimatedUnitCostUsd: 0.01, latencyClass: 'FAST', reliability: 88, resolutionSupport: [], aspectRatios: [], referenceImageSupport: false, characterConsistencySupport: false, commercialUsageNotes: 'Research/discovery only.', credentialStatus: has(env, 'TAVILY_API_KEY') ? 'LIVE' : 'NO_CREDENTIALS', enabled: true }),
