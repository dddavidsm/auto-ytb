...
export function createDefaultProviderRegistry(env: Record<string, string | undefined> = process.env): ProviderRegistry {
  return new ProviderRegistry([
    ...
    descriptor({ provider: 'higgsfield', model: env.HIGGSFIELD_VIDEO_MODEL ?? env.HF_VIDEO_MODEL ?? 'wan/v2.7/text-to-video', capabilities: ['VIDEO'], qualityScore: 88, estimatedUnitCostUsd: Number(env.HIGGSFIELD_USD_PER_SECOND ?? 0), latencyClass: 'STANDARD', reliability: 86, maxDurationSeconds: 15, resolutionSupport: ['720p', '1080p'], aspectRatios: ['16:9', '9:16', '1:1', '4:3', '3:4'], referenceImageSupport: false, characterConsistencySupport: false, seedSupport: true, commercialUsageNotes: 'Official Higgsfield API adapter. Check account/model terms and retain provider provenance.', credentialStatus: has(env, 'HF_CREDENTIALS') || hasAll(env, 'HF_API_KEY_ID', 'HF_API_KEY_SECRET') ? 'LIVE' : 'NO_CREDENTIALS', enabled: true }),
  ]);
}
