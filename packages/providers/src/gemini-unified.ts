...
export class GeminiVoiceProvider implements VoiceProvider{
  ...
  constructor(private readonly options:{apiKey:string;store:ObjectStore;model?:string;defaultVoice?:string;endpoint?:string;fetchFn?:typeof fetch;protocol?:'interactions'|'generateContent'}){}
  async synthesize(input:{text:string;voice:string;language:string}):Promise<VoiceAsset>{
    ...
    let pcm:Uint8Array;
    if(this.options.protocol==='generateContent'){
      const response=await request(fetchFn,`${baseUrl(this.options.endpoint)}/models/${encodeURIComponent(model)}:generateContent`,this.options.apiKey,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({contents:[{role:'user',parts:[{text:`Read this naturally in ${input.language}:\n${input.text}`}]}],generationConfig:{responseModalities:['AUDIO'],speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:voice}}}}})});
      const json=await response.json() as any;const inline=json?.candidates?.[0]?.content?.parts?.map((part:any)=>part?.inlineData??part?.inline_data).find((part:any)=>part?.data);if(!inline?.data)throw new Error('Gemini generateContent TTS response contained no audio data');pcm=new Uint8Array(Buffer.from(String(inline.data),'base64'));
    } else {
      ...
      const json=await response.json() as any;const audio=json?.output_audio??findBlocks(json,'audio')[0];if(!audio?.data)throw new Error('Gemini TTS response contained no audio data');pcm=new Uint8Array(Buffer.from(String(audio.data),'base64'));
    }
    const wav=wavFromPcm(pcm);const duration=Math.max(0.1,pcm.byteLength/(24000*2));const key=`gemini/voice/${Date.now()}-${Math.random().toString(36).slice(2)}.wav`;const stored=await this.options.store.put({key,contentType:'audio/wav',data:wav});return{id:key.replace(/[^a-z0-9]/gi,'-'),uri:stored.uri,mimeType:'audio/wav',bytes:stored.bytes,provider:this.name,model,language:input.language,voiceId:voice,durationSeconds:duration,alignment:approximateAlignment(input.text,duration)};
  }
}
