...
export class GeminiImageProvider implements ImageProvider{
  ...
  async generate(input:{prompt:string;aspectRatio:string;referenceUris?:string[]}):Promise<BinaryAsset>{
    const fetchFn=this.options.fetchFn??fetch;const model=this.options.model??'gemini-3.1-flash-image';const response=await request(fetchFn,`${baseUrl(this.options.endpoint)}/interactions`,this.options.apiKey,{method:'POST',headers:{'content-type':'application/json','Api-Revision':'2026-05-20'},body:JSON.stringify({model,input:input.prompt,response_format:{type:'image',mime_type:'image/png',aspect_ratio:input.aspectRatio,image_size:this.options.imageSize??'1K'}})});
    const json=await response.json() as any;const image=json?.output_image??findBlocks(json,'image')[0];if(!image?.data)throw new Error('Gemini image response contained no image data');const bytes=new Uint8Array(Buffer.from(String(image.data),'base64'));const key=`gemini/image/${Date.now()}-${Math.random().toString(36).slice(2)}.png`;const stored=await this.options.store.put({key,contentType:'image/png',data:bytes});return{id:key.replace(/[^a-z0-9]/gi,'-'),uri:stored.uri,mimeType:'image/png',bytes:stored.bytes,provider:this.name,model,metadata:{aspectRatio:input.aspectRatio,referenceCount:input.referenceUris?.length??0,synthId:true}};
  }
}
