...
async function request(fetchFn:typeof fetch,url:string,apiKey:string,init:RequestInit={},attempts=4){
  ...
  for(let i=0;i<attempts;i+=1){
    ...
    last=`${response.status}: ${(await response.text()).slice(0,800)}`;
    if(![429,500,502,503,504].includes(response.status))break;
    await sleep(Math.min(8000,500*2**i));
  }
  throw new Error(`Gemini API request failed ${last}`);
}
function findBlocks(json:any,type:string):any[]{
  const out:any[]=[];
...
function wavFromPcm(pcm:Uint8Array,sampleRate=24000,channels=1,bits=16){
  const out=new Uint8Array(44+pcm.byteLength);const view=new DataView(out.buffer);
  const text=(offset:number,value:string)=>{for(let i=0;i<value.length;i+=1)out[offset+i]=value.charCodeAt(i);};
...
function approximateAlignment(text:string,duration:number){
  const characters=[...text];const step=duration/Math.max(1,characters.length);
  return{characters,characterStartTimesSeconds:characters.map((_,i)=>i*step),characterEndTimesSeconds:characters.map((_,i)=>(i+1)*step)};
}
...
export class GeminiGoogleSearchProvider implements SearchProvider{
  ...
  async search(query:string,options?:{limit?:number;recencyDays?:number;domains?:string[]}):Promise<SearchResult[]>{
    const fetchFn=this.options.fetchFn??fetch;const limit=Math.max(1,Math.min(20,options?.limit??8));
    const constraints=[options?.recencyDays?`Prefer sources published in the last ${options.recencyDays} days.`:'',options?.domains?.length?`Prefer these domains: ${options.domains.join(', ')}.`:''].filter(Boolean).join(' ');
    const response=await request(fetchFn,`${baseUrl(this.options.endpoint)}/interactions`,this.options.apiKey,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model:this.options.model??'gemini-3.6-flash',input:`Research this query for a factual video dossier: ${query}. ${constraints} Return a concise synthesis grounded in web sources.`,tools:[{type:'google_search'}]})});
    const json=await response.json() as any;const seen=new Set<string>();const results:SearchResult[]=[];
    for(const block of findBlocks(json,'text')){
      const text=String(block.text??'');
      for(const annotation of block.annotations??[]){
        if(annotation?.type!=='url_citation'||!annotation.url||seen.has(annotation.url))continue;
        seen.add(annotation.url);const start=Number(annotation.start_index??annotation.startIndex??0),end=Number(annotation.end_index??annotation.endIndex??text.length);
        results.push({id:`gemini-${results.length}-${encodeURIComponent(annotation.url).slice(-36)}`,title:String(annotation.title||new URL(annotation.url).hostname),url:String(annotation.url),snippet:text.slice(Math.max(0,start),Math.max(start,end)).trim()||text.slice(0,500),sourceType:inferSourceType(String(annotation.url))});
...
export class GeminiVoiceProvider implements VoiceProvider{
  ...
  async synthesize(input:{text:string;voice:string;language:string}):Promise<VoiceAsset>{
    const fetchFn=this.options.fetchFn??fetch;const voice=GEMINI_VOICES.has(input.voice)?input.voice:(this.options.defaultVoice??'Kore');
    const model=this.options.model??'gemini-3.1-flash-tts-preview';
    ...
    const json=await response.json() as any;const audio=findBlocks(json,'audio')[0]??json?.output_audio;if(!audio?.data)throw new Error('Gemini TTS response contained no audio data');
    const pcm=new Uint8Array(Buffer.from(String(audio.data),'base64'));const wav=wavFromPcm(pcm);const duration=Math.max(0.1,pcm.byteLength/(24000*2));const key=`gemini/voice/${Date.now()}-${Math.random().toString(36).slice(2)}.wav`;const stored=await this.options.store.put({key,contentType:'audio/wav',data:wav});
    return{id:key.replace(/[^a-z0-9]/gi,'-'),uri:stored.uri,mimeType:'audio/wav',bytes:stored.bytes,provider:this.name,model,language:input.language,voiceId:voice,durationSeconds:duration,alignment:approximateAlignment(input.text,duration)};
  }
}
...
export class GeminiImageProvider implements ImageProvider{
  ...
  async generate(input:{prompt:string;aspectRatio:string;referenceUris?:string[]}):Promise<BinaryAsset>{
    const fetchFn=this.options.fetchFn??fetch;const model=this.options.model??'gemini-3.1-flash-image';
    const response=await request(fetchFn,`${baseUrl(this.options.endpoint)}/interactions`,this.options.apiKey,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,input:input.prompt,response_format:{type:'image',mime_type:'image/png',aspect_ratio:input.aspectRatio,image_size:this.options.imageSize??'1K'}})});
    const json=await response.json() as any;const image=findBlocks(json,'image')[0]??json?.output_image;if(!image?.data)throw new Error('Gemini image response contained no image data');const bytes=new Uint8Array(Buffer.from(String(image.data),'base64'));const key=`gemini/image/${Date.now()}-${Math.random().toString(36).slice(2)}.png`;const stored=await this.options.store.put({key,contentType:'image/png',data:bytes});
    return{id:key.replace(/[^a-z0-9]/gi,'-'),uri:stored.uri,mimeType:'image/png',bytes:stored.bytes,provider:this.name,model,metadata:{aspectRatio:input.aspectRatio,referenceCount:input.referenceUris?.length??0,synthId:true}};
  }
}
...
export class GeminiVideoProvider implements VideoProvider{
  ...
  async generate(input:{prompt:string;durationSeconds:number;aspectRatio:string;referenceUris?:string[]}):Promise<BinaryAsset>{
    const fetchFn=this.options.fetchFn??fetch;const model=this.options.model??'veo-3.1-fast-generate-preview';const base=baseUrl(this.options.endpoint);
    const create=await request(fetchFn,`${base}/models/${encodeURIComponent(model)}:predictLongRunning`,this.options.apiKey,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({instances:[{prompt:input.prompt}],parameters:{aspectRatio:input.aspectRatio,resolution:this.options.resolution??'720p',durationSeconds:Math.max(4,Math.min(8,Math.round(input.durationSeconds)))}})});
    let op=await create.json() as VideoOperation;if(!op.name)throw new Error('Gemini Veo did not return an operation name');const started=Date.now();
    while(!op.done){if(Date.now()-started>(this.options.timeoutMs??900000))throw new Error(`Gemini Veo operation ${op.name} timed out`);await sleep(this.options.pollMs??10000);const status=await request(fetchFn,`${base}/${op.name}`,this.options.apiKey);op=await status.json() as VideoOperation;}
    if(op.error)throw new Error(`Gemini Veo failed: ${op.error.message??'unknown error'}`);const uri=op.response?.generateVideoResponse?.generatedSamples?.[0]?.video?.uri;if(!uri)throw new Error('Gemini Veo completed without a video URI');const download=await request(fetchFn,uri,this.options.apiKey);const bytes=new Uint8Array(await download.arrayBuffer());const key=`gemini/video/${Date.now()}-${Math.random().toString(36).slice(2)}.mp4`;const stored=await this.options.store.put({key,contentType:'video/mp4',data:bytes});
    return{id:key.replace(/[^a-z0-9]/gi,'-'),uri:stored.uri,mimeType:'video/mp4',bytes:stored.bytes,provider:this.name,model,metadata:{aspectRatio:input.aspectRatio,requestedDurationSeconds:input.durationSeconds,generatedDurationSeconds:Math.max(4,Math.min(8,Math.round(input.durationSeconds))),resolution:this.options.resolution??'720p',referenceCount:input.referenceUris?.length??0,synthId:true}};
  }
}
