import type { AudioLibraryAsset, ProductionContentFormat, SoundtrackCue, SoundtrackPlan, VideoScript } from './types.js';
...
function cueFrom(asset:AudioLibraryAsset,startSec:number,reason:string,gainFallback:number):SoundtrackCue{
  return {assetId:asset.id,kind:asset.kind,uri:asset.uri,startSec:Math.max(0,startSec),gain:clamp(Number(asset.defaultGain??gainFallback),0.02,asset.kind==='music'?0.35:0.7),license:asset.license,rightsStatus:asset.rightsStatus,sourceUrl:asset.sourceUrl,costUsd:Math.max(0,Number(asset.costUsd??0)),reason};
}

export function selectLicensedSoundtrack(input:{script:VideoScript;contentFormat:ProductionContentFormat;catalog?:AudioLibraryAsset[];maxAudioCostUsd?:number;enableMusic?:boolean;enableSfx?:boolean}):SoundtrackPlan{
  ...
  if(input.enableMusic!==false){
    const candidates=catalog.filter((asset)=>asset.kind==='music').map((asset)=>({asset,score:assetScore(asset,input.contentFormat,moods)})).filter((item)=>Number.isFinite(item.score)).sort((a,b)=>b.score-a.score);
    const winner=candidates.find(({asset})=>spent+Math.max(0,Number(asset.costUsd??0))<=budget);
    if(winner){
      music=cueFrom(winner.asset,0,`Best cleared background match for ${moods.slice(0,4).join(', ')}`,0.16);
    ...
    }else notes.push('No budget-compatible CLEARED music asset matched; narration remains clean.');
  }
  ...
  if(input.enableSfx!==false){
    ...
    for(const beat of beatCandidates){
      ...
      const desired=[beat.purpose,beat.retentionDevice??'',beat.purpose==='reveal'?'impact':'transition'].filter(Boolean);
