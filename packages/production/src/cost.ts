...
export type CostRates = {
  ...
  aiImageUsd: number;
  aiVideoPerSecondUsd: number;
  proceduralSceneUsd: number;
  renderUsd: number;
  storageUsd: number;
};

export const DEFAULT_COST_RATES: CostRates = {
  voicePerMinuteUsd: 0.08,
  aiImageUsd: 0.04,
  aiVideoPerSecondUsd: 0.18,
  proceduralSceneUsd: 0.002,
  renderUsd: 0.15,
  storageUsd: 0.03,
};
...
export function estimateProductionCost(input: { narrationSeconds: number; scenes: Scene[]; rates?: CostRates }): number {
  ...
  const value = (input.narrationSeconds / 60) * rates.voicePerMinuteUsd
    ...
    + proceduralScenes * rates.proceduralSceneUsd
    + rates.renderUsd
    + rates.storageUsd;
