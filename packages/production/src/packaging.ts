...
export async function generatePackaging(input: { angle: StoryAngle; model: TextModel; count?: number }): Promise<PackagingVariant[]> {
  const response = await input.model.generateJson<Array<Omit<PackagingVariant, 'score'>>>({
    system: 'You design honest, high-performing YouTube packaging. Titles and thumbnails must make one clear promise and must not overclaim beyond the supplied thesis.',
    prompt: `Create ${input.count ?? 3} materially different title + thumbnail hypotheses for:\n${input.angle.title}\n${input.angle.thesis}\nPromise: ${input.angle.viewerPromise}`,
