...
export async function searchPexelsVideo(query: string, options: { apiKey?: string; fetchFn?: typeof fetch; orientation?: 'landscape' | 'portrait' | 'square'; perPage?: number } = {}): Promise<FootageSearchResult> {
  ...
  const response = await fetchFn(url, { headers: { Authorization: apiKey } });
...
export async function searchWikimediaVideo(query: string, options: { fetchFn?: typeof fetch; limit?: number } = {}): Promise<FootageSearchResult> {
  const fetchFn = options.fetchFn ?? fetch;
  ...
  const response = await fetchFn(url, { headers: { 'user-agent': 'AUTO-YTB FootageAgent (rights-aware research client)' } });
  if (!response.ok) throw new Error(`Wikimedia video search failed: ${response.status}`);
  ...
  const candidates = Object.values(json.query?.pages ?? []).flatMap((page) => {
    ...
    return [{ id: `commons-video-${page.pageid}`, sourceUrl: `https://commons.wikimedia.org/wiki/${encodeURIComponent(String(page.title ?? '').replaceAll(' ', '_'))}`, provider: 'Wikimedia Commons', rightsTier: blocked ? 'BLOCKED' as const : 'PUBLISHABLE_WITH_ATTRIBUTION' as const, durationSeconds: 0, usableDurationSeconds: 0, width: Number(info.width ?? 0), height: Number(info.height ?? 0), sourceAudio: true, sourceKey: `commons:${page.pageid}`, visualFingerprint: `commons:${page.pageid}`, metadata: { downloadUrl: info.url, license, creator: String(meta.Artist?.value ?? meta.Credit?.value ?? '').replace(/<[^>]*>/g, '').trim() } } satisfies MovingVideoCandidate & { metadata: Record<string, unknown> }];
  });
