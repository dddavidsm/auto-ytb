...
export async function runContentPipeline(input: {
  ...
  for(let attempt=0;attempt<=maxRepairs;attempt+=1){
    ...
    revisionGuidance=[
      ...
      executionPlan.researchRequired
        ...
        :'Preserve the original premise and any canonical character/world constraints. Do not turn fictional/generated events into claimed real-world evidence. Every beat must create visible progress toward the payoff.',
    ];
  ...
  const estimatedCostUsd = estimateProductionCost({ narrationSeconds: voice?.durationSeconds??0, scenes }) + (isShort ? 0 : packaging.length * 0.12);
  event('PLAN', `Archetype visual mix: ${Object.entries(visualMix).map(([kind,count]) => `${kind}=${count}`).join(', ')} · generative bias ${executionPlan.generativeSpendBias.toFixed(2)}`);
  ...
  event('PLAN', `Metered pre-render spend $${manifest.actualCostUsd.toFixed(4)} · editorial $${editorialCostUsd.toFixed(4)} · media $${mediaCostUsd.toFixed(4)} · research ${executionPlan.researchRequired?'ON':'OFF'} · voice ${executionPlan.voiceRequired?'ON':'OFF'}`);
