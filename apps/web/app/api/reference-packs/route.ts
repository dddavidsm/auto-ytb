...
export async function POST(request: Request) {
  ...
  for (const row of rows) await query('insert into reference_pack_items (reference_pack_id,video_id,external_video_id,channel_id,role,evidence,outlier_score,selected_because) values ($1,$2,$3,$4,$5,$6::jsonb,$7,$8)', [pack[0].id, row.id, row.youtube_video_id, row.channel_id, 'USER_SELECTED', JSON.stringify([]), null, 'Selected in Reference Lab V2']);
