...
type Result = { runId: string; pipeline: string; artifactRoot: string; brief: { briefId: string; format: string; durationSec: number; qualityMode: string }; quote: { note: string; knownPaidCostUsd: number }; progress: string[] };
...
export default function CreatePage() {
  ...
  return <div className="content"><header className="topbar"><div><p className="eyebrow">One-prompt studio</p><h1>Crear un vídeo</h1></div><Link href="/" className="pill info">← Portfolio</Link></header><main className="main">
    ...
    {result ? <section className="card" style={{ maxWidth: 900, margin: '20px auto' }}><div className="row"><div><p className="eyebrow">Producción autónoma iniciada</p><h2>{result.brief.briefId}</h2></div><span className="pill good">{result.brief.format}</span></div><div className="kpis"><div className="kpi"><span>Duración</span><strong>{result.brief.durationSec}s</strong></div><div className="kpi"><span>Calidad</span><strong>{result.brief.qualityMode}</strong></div><div className="kpi"><span>Coste conocido</span><strong>${result.quote.knownPaidCostUsd.toFixed(2)}</strong></div></div><p className="muted">Mismo pipeline canónico que las pruebas autónomas. Run: <code>{result.runId}</code></p><p className="muted">{result.quote.note}</p><div className="list">{result.progress.map((stage) => <div className="list-item" key={stage}><span>{stage}</span><span className="pill good">en cola</span></div>)}</div></section> : null}
  </main></div>;
}
