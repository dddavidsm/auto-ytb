...
export default function LoginPage(){
  ...
  return <main className="login-shell"><section className="login-card"><div className="brand-mark">A</div><div><p className="eyebrow">AUTO-YTB</p><h1>Control plane</h1><p className="muted">Producción, revisión, costes, rendimiento y aprendizaje creativo en un único panel.</p></div><a href="/api/auth/google" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10,border:'1px solid #dadce0',borderRadius:12,padding:'12px 14px',fontWeight:650,background:'#fff'}}><span style={{fontWeight:800,color:'#1a73e8'}}>G</span><span>Continuar con Google</span></a><div style={{display:'flex',alignItems:'center',gap:10,color:'#5f6368',fontSize:12}}><span style={{height:1,background:'#e0e3e7',flex:1}}/><span>o usa la clave de emergencia</span><span style={{height:1,background:'#e0e3e7',flex:1}}/></div><form onSubmit={submit}><label htmlFor="token">Clave de acceso</label><input id="token" name="token" type="password" autoComplete="current-password" required placeholder="••••••••••••"/><button className="primary" disabled={busy}>{busy?'Entrando…':'Acceder'}</button>{error?<p className="error">{error}</p>:null}</form><p className="fine">Google es el acceso normal. La clave local queda como fallback. La sesión se guarda en una cookie HttpOnly firmada.</p></section></main>;
}
