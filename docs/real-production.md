...
Recognized variables: `DATABASE_URL`, optional `DATABASE_SSL=true`, `TAVILY_API_KEY`, `OPENAI_API_KEY`/`OPENAI_MODEL`, `GEMINI_API_KEY` or `GOOGLE_API_KEY`/`GEMINI_MODEL`, `VOICE_API_KEY` (or legacy `ELEVENLABS_API_KEY`), and `RUNWAY_API_KEY`/`RUNWAY_VIDEO_MODEL`.
